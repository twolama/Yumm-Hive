  ...
  <div className="bg-[color:var(--surface-1)] rounded-3xl px-5 py-4 border border-[color:var(--border-soft)]/80 relative overflow-hidden min-h-[146px]">
    ...
    <div className="relative z-10 flex flex-col h-full justify-between">
      <div className="flex justify-between items-start mb-5">
        <h3 className="text-xs font-bold text-[color:var(--muted)] uppercase tracking-[0.18em]">
          {title}
        </h3>
...
const HiveNode = ({
  ...
  const statusColors = {
    ...
    warning:
      "bg-[color:var(--status-warning)] shadow-[0_0_10px_rgba(246,160,33,0.3)]",
    critical:
      "bg-[color:var(--status-critical)] shadow-[0_0_10px_rgba(239,68,68,0.3)]",
  };

  return (
    <Link
      href={`/dashboard/my-hives/${id}`}
      className="bg-[color:var(--surface-1)] border border-[color:var(--border-soft)]/70 hover:border-[color:var(--status-optimal)]/50 transition-all duration-300 rounded-full py-2 px-4 flex items-center justify-between cursor-pointer group hover:scale-[1.03] active:scale-[0.97]"
    >
      <span className="text-sm font-bold text-[color:var(--muted)] group-hover:text-foreground transition-colors">
        #{id}
      </span>
      <div
        className={cn(
...
const FeedCard = ({
  ...
  const tagStyles: Record<string, string> = {
    ...
    AUTOMATED:
      "bg-[color:var(--status-automated)]/20 text-[color:var(--status-automated-text)]",
  };

  return (
    <Link href={href} className="block group">
      <div className="bg-[color:var(--surface-2)] rounded-2xl p-4 md:p-5 flex flex-col sm:flex-row sm:items-center justify-between border border-[color:var(--border-soft)]/60 cursor-pointer hover:border-[color:var(--primary)]/30 transition-all hover:bg-[color:var(--surface-3)]/50 gap-4">
  ...
  return (
    <div className="mx-auto w-full max-w-[1180px] space-y-7 pb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard
          title="TOTAL HIVES"
          value={HIVE_SUMMARIES.length.toString()}
          highlight="+3 this month"
          icon={RadioTower}
          progressColor="bg-[color:var(--status-optimal)]"
        />
        <StatCard
          title="SEASON YIELD"
          value="1,280"
        ...
        <StatCard
          title="AMBIENT TEMP"
          value="24.5"
          highlight="Stable in valley"
          unit="C"
          icon={Thermometer}
          progressColor="bg-[color:var(--muted)]/40"
        />
      </div>
      ...
      <section className="bg-[color:var(--feed-bg)]/10 border border-[color:var(--feed-border)]/40 rounded-3xl p-4 md:p-5">
        <div className="flex items-end justify-between mb-4">
          <div>
            <h2 className="text-3xl leading-tight font-black text-foreground">
              Hive Fleet Health
            </h2>
            ...
            .map((hive) => {
              ...
              return (
                <HiveNode
                  key={hive.id}
                  id={hive.id}
                  status={statusMap[hive.status] || "optimal"}
                />
              );
            })}
        </div>
      ...
      <section className="bg-[color:var(--feed-bg)] border border-[color:var(--feed-border)] rounded-3xl p-4 md:p-6 relative overflow-hidden">
        ...
        <div className="flex items-start sm:items-center gap-4 mb-6 relative z-10">
          ...
          <div>
            ...
            <p className="text-sm text-[color:var(--primary)]">
              AI-driven actionable insights for your apiary
            </p>
          </div>
        </div>
